...
#Fit the k-nn algorithm to the train data and inspect the obtained model.
knn_fit1 <- model_knn %>%
  fit(intentional_cause ~ district + TemperatureCMax + WindkmhInt + TemperatureCAvg + TemperatureCMin + village_area + extinction_hour + farming_area + village_veget_area, data = train, na.action = na.exclude)#prever intentional_cause em relação a todas as variaveis
knn_fit1
...
# Build the model
model <- lm(intentional_cause ~ TemperatureCMax + WindkmhInt + TemperatureCAvg + TemperatureCMin + village_area + extinction_hour + farming_area + village_veget_area, data = fire_Train_Data, na.action = na.exclude)#prever intentional_cause em relação a todas as variaveis
model

model_lm <- linear_reg(engine="lm") 
lm_fit13 <- model_lm %>%
  fit(intentional_cause ~ TemperatureCMax + WindkmhInt + TemperatureCAvg + TemperatureCMin + village_area + extinction_hour + farming_area + village_veget_area, data = fire_Train_Data, na.action = na.exclude)#prever intentional_cause em relação a todas as variaveis
