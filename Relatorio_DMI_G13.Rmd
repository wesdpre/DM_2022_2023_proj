...
#  .metric .estimator .estimate
#  <chr>   <chr>          <dbl>
#1 rmse    standard      0.440 
#2 rsq     standard      0.0596
#3 mae     standard      0.388 
...
Observando estas métricas, o modelo com melhor resultado foi *Multiple Linear Regression Model*, com o fit _fit(intentional_cause ~ district + TemperatureCMax, data = fire_train)_, pois os resultados foram:
- rmse    standard      0.433 
- rsq     standard      0.0878
- mae     standard      0.379 
No entanto, o modelo *KNN Model* foi o melhor. Este obteve uma *accuracy* de 69%. Testou-se o modelo usando _recipe()_ e sem usar, esta função precisa ser estimada antes de ser aplicada aos dados. A maioria das etapas da _recipe()_ possui quantidades específicas que devem ser calculadas ou estimadas. Por exemplo, _step_normalize()_ precisa calcular a média do conjunto de treinamento para as colunas selecionadas.
Após averiguar a melhor métrica a ter em consideração, guardou-se os dados num ficheiro _csv_, e, este foi colocado no kaggle a sua previsão, sendo que com a _recipe()_ não foi obtido pontução suficiente, contudo, sem esse pre-processamento a previsão obteve pontuação suficiente.
