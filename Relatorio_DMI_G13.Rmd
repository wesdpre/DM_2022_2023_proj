...
#### 2. Pré-processamento de dados e Análise exploratória de dados
...
Para este trabalho foi também cedido um conjunto de dados de teste, este, será para posteriormente ser colocado o valor previsto por um modelo de machine learning que será posteriormente estudado.
Contudo, este conjunto de dados apresenta alguns dados em falta como a region que apresentava dois valores nulos, estes foram preenchidos como os de dados de treino.
Também se usou a função fill para preencher as colunas "extinction_date", "extinction_hour", "firstInterv_date" e "firstInterv_hour".
...
Desta forma, para cada dia e localização disponíveis no conjunto de dados principal foi possível anexar mais um conjunto de características tanto ao conjunto *fire_Train_Data*, bem como, ao *fire_Test_Data*.
...
A coluna *region* foi também eliminada pois apresentava 1216 valores com o caracter "-".
...
Para o conjunto *fire_Test_Data*, procedeu-se da mesma forma, sendo que, neste conjunto de dados não foi possivel apagar qualquer linha uma vez que a competição do kaggle assim o exigia.
Assim, apenas foram acrescentadas novas características ao conjunto de dados.
Para os casos em que existem valores nulos então foi usado o fill com deslocamento para cima e para baico na linha.
...
A coluna *id* foi eliminada pois apenas identifica o caso não traz valor para o conjunto de dados, assim sendo, esta deve ser eliminada.
Bem como, as colunas *lon* e *lat* já não acrescentariam valor, assim, também foram eliminadas.
Uma vez que não é possível reduzir o número de linhas deste conjunto de dados no final, o valores nulos foram preenchidos pelos valores das linhas acima e abaixo.
...
Pode-se observar também que as temperaturas estão diretamente relacionadas,o é, instintivamente natural.
Verificou-se ainda uma correlção entre *fire_duration* e *village_area* e *vegetation_area*, também algo bastante natural, tendo em conta que quão maior for a área de vegetação maior será a proporção que o fogo pode tomar, e, por consequência, o tempo de duração.
...
Para poder estudar os modelos preditivos o conjunto de dados de treino tem que ser dividido, para que se possa ensinar o modelo e depois testar para saber a sua performance.

Assim, dividiu-se o data set em:

-   Conjunto de treino (70%) - um subconjunto para treinar/ensinar os modelos;
-   Conjunto de teste (30%) - um subconjunto para testar os modelos treinados e obter métricas de performance.
...
O objectivo final é averiguar qual o modelo melhor para prever se o fogo foi posto ou não.
Decidiu-se experimentar o data set em quatro modelos diferentes, para ver qual deles será o melhor ,e, por consequência, o que faz melhores previsões.
...
#2 rsq     standard      0.0878
#3 mae     standard      0.379 
```
...
Depois de feitas as previsões no modelo, avaliou-se o seu desempenho, como se pode ver nos resultados (comentários), para avaliar, teve-se em conta as métricas rmse, r2 e mae.
O erro de previsão RMSE (Root Mean Squared Error), este representa a diferença média entre os valores conhecidos observados nos dados de teste e os valores de resultados previstos pelo modelo.
Quanto *menor* o valor, melhor o modelo.
O R-square (R2), que representa a correlação entre os valores observados e os valores previstos.
Quanto *maior* o valor, melhor o modelo.
Erro absoluto médio (MAE) é o erro absoluto médio entre os valores reais e previstos.
Quanto *menor* o valor, mais preciso é o modelo.
Observando estas métricas, um modelo com bons resultados foi *Multiple Linear Regression Model*, com o fit _fit(intentional_cause ~ district + TemperatureCMax, data = fire_train)_, pois os resultados foram:
- rmse    standard      0.433 
- rsq     standard      0.0878
- mae     standard      0.379
Testou-se também, o *Lasso Regression Model*, cujos resultados foram:
-  rmse    standard      0.433 
-  rsq     standard      0.0892
-  mae     standard      0.377 
No entanto, é de salientar, que o modelo com melhor pontuação (kaggle) foi *Ridge Regression Model*, onde mae é menor e rsq maior.
-  rmse    standard      0.433 
-  rsq     standard      0.0892
-  mae     standard      0.377 
No entanto, o modelo *KNN Model* obteve uma *accuracy* de 69%. Testou-se o modelo usando _recipe()_ e sem usar, esta função precisa ser estimada antes de ser aplicada aos dados. A maioria das etapas da _recipe()_ possui quantidades específicas que devem ser calculadas ou estimadas. Por exemplo, _step_normalize()_ precisa calcular a média do conjunto de treinamento para as colunas selecionadas.
Após averiguar a melhor métrica a ter em consideração, guardou-se os dados num ficheiro _csv_, e, este foi colocado no kaggle a sua previsão, sendo que com a _recipe()_ não foi obtido pontução suficiente, contudo, sem esse pre-processamento a previsão obteve pontuação suficiente. Também foi colocado no kaggle as pevisões feitas por *Ridge Regression Model*, *Lasso Regression Model* e *Multiple Linear Regression Model*, de salientar que foi *Ridge Regression Model* que obteve ainda melhor pontução.


#### 5. Conclusão

Deve concluir-se que este trabalho abordou diversas temáticas apresentadas nas aulas de Data Mining I. Apresentaram-se algumas adversidades, normais, pois o conjunto de dados teve que ser pré-processado, algumas decisões foram tomadas com intuito de melhorar o conjunto de dados mas que, por consequência, o diminui.
Foi possível aplicar alguns modelos de previsão e, contudo, é de salientar a dificuldade em obter bons modelos de previsão. No entanto, usando R, uma linguagem orientada a Data Science, foi possível prever se foi fogo posto.
Deve salientar-se, no entanto, que o trabalho provou-se muito interessante e com um final positivo.
Por fim, completou-se o trabalho entregando o ficheiro com a previsão na plataforma Kaggle.
